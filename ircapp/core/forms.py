        ...
        if DownloadOngoing.objects.all().count() > 0:
            if DownloadOngoing.objects.latest('id').active and DownloadOngoing.objects.latest(
                ...
                raise forms.ValidationError(
                    'Cannot change the download directory while downloading. Please wait for your download to finish.',
                    code='invalid')
        if download_path:
            if not os.path.exists(download_path):
                raise forms.ValidationError(
                    'Indicated path could not be found. Please make sure you indicate the full directory path.',
                    code='invalid')
